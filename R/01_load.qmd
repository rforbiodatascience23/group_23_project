...
As there are 3 duplicates, and the function `make.unique` will add a .1 t the end of the name to make it unique we should identify which samples are the duplicates. This is of high importance when later on we joined the proteomic data with the clinical data, as we will need to have the same clinical information for these duplicates.
...
Now, taking into account the duplicates, we will modify the clinical table to include the rows corresponding to the duplicates, using the names f the duplicates, so that after the join that it is intended to do, these samples do not contain NAs in their clinical information.
...
```{r}
final_data %>%
  group_by(Complete.TCGA.ID = sub("\\.1$", "", Complete.TCGA.ID))
```
...
# Iterate through columns ending with ".1"
for (col in duplicates) {
  # Extract the information without ".1"
  original <- gsub("\\.1$", "", col)
  
  # Duplicate the columns without ".1"
  final_data[[original]] <- final_data[[col]]
}
```
