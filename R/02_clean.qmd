...
It can be interesting to see how many missing values are there per sample. If there are some samples with a great amount of NA values, maybe they should not be considered.
...
```{r}
clean_prot_cli <- clean_prot_cli |>
  mutate_at(vars(starts_with("XP") | starts_with("YP") | starts_with("NP")),
            ~replace_na(.,
                        mean(.,
                            na.rm = TRUE)))
```
