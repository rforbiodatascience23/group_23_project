...
As done before, the variables are renamed to allow a cleaner code. In this case, they are not restored to their original names, as the new ones are sufficiently informative. In the new column, `TNBC` , individuals with TNBC will store the value 0, while the rest will be assigned 1.
...
The Node.Coded column will be renamed to get the name Node_Cancer. Additionally, the values that are coded as "Positive" will receive the value 1, and those coded as "Negative" will receive the value 0. This minor changes are with regards to further Differential Expression Analysis.
